...
from database import invest_reflection as invest_reflection_table
import logging
...
if st.session_state.view_id:
    ...
    doc = invest_reflection_table.get_doc_by_id(doc_id)
    if doc:
        st.markdown(f"""
        <div class="modal-overlay">
            <div class="modal-box">
                <div style="display:flex; justify-content:space-between;">
                    <p><b>📊 {doc['title']} </b></p>
                    <a href="/invest_reflection" target="_self" style="padding:4px 15px; background:#444; color:white; border-radius:5px; text-decoration:none;">关闭</a>
                </div>
                <hr>
...
if st.session_state.page == 'list':
    ...
    with st.container():
        ...
        if search_title != st.session_state.search_title or search_tag != st.session_state.search_tag :
            ...
            st.session_state.current_page = 1
            st.rerun()
    total_count = invest_reflection_table.query_doc_count(tag_filter=search_tag,title_filter=search_title)
    PAGE_SIZE=20
    ...
    docs = invest_reflection_table.query_doc_by_page(tag_filter=search_tag,title_filter=search_title,page=st.session_state.current_page-1, page_size=PAGE_SIZE)
...
# ==========================
    ...
    doc = invest_reflection_table.get_doc_by_id(edit_id)
    ...
    if not doc:
        ...
        with col1:
            ...
            if st.button("💾 保存更新", type="primary"):
                ...
                invest_reflection_table.update_doc({
                    "id": edit_id,
                    "title": title,
    ...
    with col1:
        if st.button("✅ 保存文档", type="primary"):
            if not title or not content:
                ...
                # 插入数据库
                invest_reflection_table.add_doc(
                    title=title,
                    tags=",".join((tags or "").split()),
                    content=content
                )
