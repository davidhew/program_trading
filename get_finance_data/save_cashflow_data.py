...
def save_cashflow_data(ts_code:str,df:pd.DataFrame):
        ...
        df.to_csv(config.CHINA_STOCK_FINANCE_DATA_DIR+ts_code+cashflow_suffix,index=True)
...
def should_update_data(ts_code:str):
    ...
    path = Path(config.CHINA_STOCK_FINANCE_DATA_DIR + ts_code + cashflow_suffix)
...
def get_cashflow_data(ts_code):
    path = Path(config.CHINA_STOCK_FINANCE_DATA_DIR+ts_code+cashflow_suffix)
    if not path.exists() or not path.is_file():
        logger.error("%s stock data cashflow file not exist! pls check reason",ts_code)
