...
def get_doc_table():
    ...
    return db_manager.get_database()[('invest_reflection')]


# --- INSERT (插入) ---
# 插入时不需要预先定义字段，dataset 会根据字典的 key 自动生成列
@db_monitor(db_manager.get_database())
def add_doc(title, tags,content):
    table = get_doc_table()
    table.insert({
        'title': title,
...
# ==============================
@db_monitor(db_manager.get_database())
def query_doc_count(tag_filter=None, title_filter=None):
    sql = "SELECT COUNT(*) AS total FROM invest_reflection WHERE 1=1"
    params = {}
...
# ==============================
@db_monitor(db_manager.get_database())
def query_doc_by_page(tag_filter=None, title_filter=None, page=0, page_size=20):
    offset = page * page_size  # 因为页码从0开始，直接乘

    sql = "SELECT * FROM invest_reflection WHERE 1=1"
    params = {}
