...
def get_stock_data_batches(batch_size=10):
    ...
    stock_data_folder = Path(config.CHINA_STOCK_DATA_DIR)
...
def get_stock_data(ts_code):
    path = Path(config.CHINA_STOCK_DATA_DIR+ts_code)
    if not path.exists() or not path.is_file():
        logger.error("%s stock data file not exist! pls check reason",ts_code)
